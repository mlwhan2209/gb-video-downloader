...
Import-Module ./gb-scraper.psm1 -Force
...
## set to key/value pair then place in data.log file ##
$entryData = @()
$i = 0
...
foreach ($entry in $var.results){
    ...
    elseif ($entry.high_url) {
        ...
        Invoke-WebRequest -URI "$($entry.high_url)$key" -Outfile "./$($entry.name).mp4"
    }
    elseif ($entry.low_url) {
        Write-Output "Downloading Low version of $($entry.name)"
        Invoke-WebRequest -URI "$($entry.low_url)$key" -Outfile "./$($entry.name).mp4"
    }
    else {
